                                    ...
                                    <tbody class="">
                                        ...
                                        <tr class="border-b">
                                            ...
                                            <td class="px-4 py-3 flex items-center justify-end">
                                                ...
                                                <div id="dropdown-{{ $project->id }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow">
                                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="apple-imac-27-dropdown-button">
                                                        <li>
                                                            <a href="{{ route('admin.projects.show', $project -> id)}}" class="block py-2 px-4 hover:bg-gray-100">Show</a>
                                                        </li>
                                                        <li>
                                                            <a href="{{ route('admin.projects.edit', $project -> id)}}" class="block py-2 px-4 hover:bg-gray-100">Edit</a>
                                                        </li>
                                                    </ul>
                                                    <div class="py-1">
                                                        <form id="delete-form-{{ $project->id }}" action="{{ route('admin.projects.destroy', $project->id) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <a href="{{ route('admin.projects.destroy', $project->id) }}" class="block py-2 px-4 text-sm font-bold text-red-700 hover:bg-red-200" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $project->id }}').submit();">
                                                                Delete
                                                            </a>
                                                        </form>
                                                    </div>
